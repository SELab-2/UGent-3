...
type ExtendedPickersDayProps = PickersDayProps<Dayjs> & {
...
const DeadlineInfo: React.FC<DeadlineInfoProps> = ({
  ...
  const deadlinesOnSelectedDay = deadlines.filter(
    (project) =>
      project.deadline && dayjs(project.deadline).isSame(selectedDay, "day"),
  );
    ...
    <div>
      {deadlinesOnSelectedDay.length === 0 ? (
        <Card style={{ margin: "10px 0" }}>
          <CardContent>
            <Typography variant="body1">{t("noDeadline")}</Typography>
          </CardContent>
        </Card>
      ) : (
...
const handleMonthChange = (
  ...
  projects.map((project) => {
    if (
      project.deadline &&
      project.deadline.getMonth() == date.month() &&
      project.deadline.getFullYear() == date.year()
    ) {
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DateCalendar
                ...
                onMonthChange={(date: Dayjs) => {
                  handleMonthChange(date, projects, setHighlightedDays);
                }}
