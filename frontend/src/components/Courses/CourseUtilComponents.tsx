  ...
  return (
    <Grid item xs={12} marginLeft="2rem">
      <Grid container direction="row" spacing={2}>
        ...
        <SearchBox
          label={"ufora id"}
          searchTerm={uforaIdFilter}
          handleSearchChange={handleUforaIdFilterChange}
        />
        <SearchBox
          label={"teacher"}
          searchTerm={teacherNameFilter}
          handleSearchChange={handleTeacherNameFilterChange}
        />
      </Grid>
      <EmptyOrNotFilteredCourses
          ...
          <Grid item key={index} xs={12} sm={6} md={4} lg={2}>
            <Card variant="outlined" style={{ width: "14vw" }}>
              ...
              <CardContent style={{ margin: "0.5rem", height: "12vh" }}>
                ...
                <EmptyOrNotProjects
                  projects={projects[getIdFromLink(course.course_id)]}
                  noProjectsText={t("noProjects")}
                />
              </CardContent>
              <CardActions>
                <Link to={`${getIdFromLink(course.course_id)}`}>
        ...
        {projects.slice(0, 3).map((project) => {
          ...
          return (
            <Grid item key={project.project_id}>
              ...
              >
                <EpsilonTypography
                  text={`${project.title} ${timeLeft ? ` - ${timeLeft}` : ""}`}
                />
              </Link>
            </Grid>
          );
